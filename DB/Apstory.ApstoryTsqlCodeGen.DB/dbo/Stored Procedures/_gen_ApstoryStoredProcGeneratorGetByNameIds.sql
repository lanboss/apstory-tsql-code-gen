...
CREATE   PROCEDURE [dbo].[*gen_ApstoryStoredProcGeneratorGetByNameIds]
...
DEALLOCATE TableCol;
    ...
    BEGIN
        ...
        SET @QueryToExec = @QueryToExec +  N'  (@Uniqueidentifiers udtt_Uniqueidentifiers READONLY' + IIF(@ISACTIVE_SEL = 1,', @IsActive bit=NULL','') + ')' + CHAR(13) + CHAR(10);
        ...
        BEGIN
    		...
    		SET @QueryToExec = @QueryToExec +  N'    SELECT * FROM [' + @Schema + '].[' + @TableName + '] WHERE [' + @PRIMARY_KEY + '] IN (Select Id FROM @Uniqueidentifiers);' + CHAR(13) + CHAR(10) 
    		SET @QueryToExec = @QueryToExec +  N'  ELSE' + CHAR(13) + CHAR(10)
    		SET @QueryToExec = @QueryToExec +  N'    SELECT * FROM [' + @Schema + '].[' + @TableName + '] WHERE [' + @PRIMARY_KEY + '] IN (Select Id FROM @Uniqueidentifiers) AND IsActive = @IsActive;' + CHAR(13) + CHAR(10) 
    	END
    	ELSE
    		SET @QueryToExec = @QueryToExec +  N'    SELECT * FROM [' + @Schema + '].[' + @TableName + '] WHERE [' + @PRIMARY_KEY + '] IN (Select Id FROM @Uniqueidentifiers);' + CHAR(13) + CHAR(10) 
