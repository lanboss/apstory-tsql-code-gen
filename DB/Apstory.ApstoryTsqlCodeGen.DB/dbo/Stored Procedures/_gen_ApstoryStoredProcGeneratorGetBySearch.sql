...
exec [*gen_ApstoryStoredProcGeneratorGetBySearch] 'Company', @QueryToExec output
select @QueryToExec
*/

CREATE   PROCEDURE [dbo].[*gen_ApstoryStoredProcGeneratorGetBySearch]
...
WHILE @@FETCH_STATUS = 0
    BEGIN
        ...
        BEGIN
			SET @CONTAINS=@CONTAINS + 'CONTAINS(' + @ColumnName + ',@SearchCriteria) OR '
...
DEALLOCATE TableCol;
	...
	IF (@HASINDEX > 0)
	BEGIN
		SET @QueryToExec = N'/****** Object:  StoredProcedure [' + @Schema + '].[zgen_' + @TableName + '_GetBySearch] ******/' + CHAR(13) + CHAR(10) 
		...
		SET @QueryToExec = @QueryToExec +  N'CREATE OR ALTER PROCEDURE [' + @Schema + '].[zgen_' + @TableName  + '_GetBySearch]' + CHAR(13) + CHAR(10);
