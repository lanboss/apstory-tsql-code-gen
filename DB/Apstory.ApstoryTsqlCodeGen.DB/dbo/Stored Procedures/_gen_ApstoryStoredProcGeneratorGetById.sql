...
exec [*gen_ApstoryStoredProcGeneratorGetById] 'Account', @QueryToExec output
select @QueryToExec
*/

CREATE   PROCEDURE [dbo].[*gen_ApstoryStoredProcGeneratorGetById]
...
--Check if tabe has IsActive Column
...
DEALLOCATE TableCol;
    ...
    SET @QueryToExec = N'/****** Object:  StoredProcedure [' + @Schema + '].[zgen_' + @TableName + '_GetById] ******/' + CHAR(13) + CHAR(10) 
    ...
    SET @QueryToExec = @QueryToExec +  N'CREATE OR ALTER PROCEDURE [' + @Schema + '].[zgen_' + @TableName  + '_GetById]' + CHAR(13) + CHAR(10);
    SET @QueryToExec = @QueryToExec +  N'  (@' + @PRIMARY_KEY + ' ' + @PrimaryKeyDataType + IIF(@ISACTIVE_SEL = 1,', @IsActive bit=NULL','') + ')' + CHAR(13) + CHAR(10);
    ...
    BEGIN
        ...
        SET @QueryToExec = @QueryToExec +  N'        SELECT * FROM [' + @Schema + '].[' + @TableName + '] ORDER BY [' + @PRIMARY_KEY + '] ' + @OrderByDir + ';' + CHAR(13) + CHAR(10) 
        SET @QueryToExec = @QueryToExec +  N'      ELSE' + CHAR(13) + CHAR(10) 
        SET @QueryToExec = @QueryToExec +  N'        SELECT * FROM [' + @Schema + '].[' + @TableName + '] WHERE [IsActive] = @IsActive ORDER BY [' + @PRIMARY_KEY + '] ' + @OrderByDir + ';' + CHAR(13) + CHAR(10) 
    ...
    BEGIN
		...
		SET @QueryToExec = @QueryToExec +  N'      ELSE' + CHAR(13) + CHAR(10)
		SET @QueryToExec = @QueryToExec +  N'	    SELECT * FROM [' + @Schema + '].[' + @TableName + '] WHERE [' + @PRIMARY_KEY + '] = @' + @PRIMARY_KEY + ' AND IsActive = @IsActive;' + CHAR(13) + CHAR(10) 
	END
	ELSE
		SET @QueryToExec = @QueryToExec +  N'	    SELECT * FROM [' + @Schema + '].[' + @TableName + '] WHERE [' + @PRIMARY_KEY + '] = @' + @PRIMARY_KEY + ';' + CHAR(13) + CHAR(10) 
	SET @QueryToExec = @QueryToExec +  N'    END' + CHAR(13) + CHAR(10) 
