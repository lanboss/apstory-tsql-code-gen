...
exec [*gen_ApstoryStoredProcGeneratorDelSft] '', @QueryToExec output
select @QueryToExec
*/

CREATE   PROCEDURE [dbo].[*gen_ApstoryStoredProcGeneratorDelSft]
...
DEALLOCATE TableCol;
	...
	--Print Soft Delete Statement
	IF @ISACTIVE_SEL = 1
    BEGIN	
		SET @QueryToExec = N'/****** Object:  StoredProcedure [' + @Schema + '].[zgen_' + @TableName + '_DelSft] ******/' + CHAR(13) + CHAR(10) 
		SET @QueryToExec = @QueryToExec +  @LEGEND;
		SET @QueryToExec = @QueryToExec +  N'-- Description    : Soft Delete ' + @TableName + CHAR(13) + CHAR(10) 
		SET @QueryToExec = @QueryToExec +  N'-- ===================================================================' + CHAR(13) + CHAR(10) 
		SET @QueryToExec = @QueryToExec +  CHAR(13) + CHAR(10) 
		SET @QueryToExec = @QueryToExec +  N'CREATE OR ALTER PROCEDURE [' + @Schema + '].[zgen_' + @TableName  + '_DelSft]' + CHAR(13) + CHAR(10);
		...
		SET @QueryToExec = @QueryToExec +  N'  UPDATE [' + @Schema + '].[' + @TableName + '] SET IsActive = 0 WHERE [' + @PRIMARY_KEY + ']=@' + @PRIMARY_KEY + ';' + CHAR(13) + CHAR(10) + CHAR(13) + CHAR(10)     
