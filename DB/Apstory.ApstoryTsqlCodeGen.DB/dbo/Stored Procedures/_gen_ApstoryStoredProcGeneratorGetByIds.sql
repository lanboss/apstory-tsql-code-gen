...
exec [*gen_ApstoryStoredProcGeneratorGetByIds] 'UserClub', @QueryToExec output
select @QueryToExec
*/

CREATE   PROCEDURE [dbo].[*gen_ApstoryStoredProcGeneratorGetByIds]
...
DEALLOCATE TableCol;
  ...
  BEGIN
    ...
    SET @QueryToExec = N'/****** Object:  StoredProcedure [' + @Schema + '].[zgen_' + @TableName + '_GetByIds] ******/' + CHAR(13) + CHAR(10) 
    SET @QueryToExec = @QueryToExec +  @LEGEND;
    SET @QueryToExec = @QueryToExec +  N'-- Description    : Select By Ids ' + @TableName + CHAR(13) + CHAR(10) 
    SET @QueryToExec = @QueryToExec +  N'-- ===================================================================' + CHAR(13) + CHAR(10) + CHAR(13) + CHAR(10)     
    SET @QueryToExec = @QueryToExec +  N'CREATE OR ALTER PROCEDURE [' + @Schema + '].[zgen_' + @TableName  + '_GetByIds]' + CHAR(13) + CHAR(10);
	SET @QueryToExec = @QueryToExec +  N'  (' + @PARAMETERS + IIF(@ISACTIVE_SEL = 1,',@IsActive bit=NULL','') + ',@SortDirection varchar(5)=''ASC'')' + CHAR(13) + CHAR(10);    
    ...
    BEGIN
	   ...
	   SET @QueryToExec = @QueryToExec +  N'        ORDER BY ' + CHAR(13) + CHAR(10)
